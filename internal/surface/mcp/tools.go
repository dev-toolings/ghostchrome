...
// toolDefs is the single source of truth for the MCP tool surface: both
// registerTools and Tools() read it, so the registered set and the enumerated
// set can never drift. Mirrors internal/runtime's handlers table.
//
// The set is intentionally small — snapshot covers preview/extract/errors,
// type covers fill_form, eval is the escape hatch for everything else.
