...
ghostchrome/
  ...
  sdk/npm/               wrappers de distribution (ex packages/npm/)
...
|---|---|---|---|
| 0 | Hygiène | retirer `docs/` et `*.yaml` de `.gitignore`, versionner `docs/`, réécrire `docs/architecture.md` en ASCII 7 bits, `package.json` racine avec workspaces Bun (`sdk/typescript`, `sdk/npm/*`) | `git ls-files docs \| wc -l` >= 11 ; `bun install` à la racine |
...
| 5 | Séparer distribution / recipes | `packages/npm` -> `sdk/npm`, `packages/<site>` -> `recipes/`, mettre à jour `.gitignore` et le tag de build | `go build -tags recipes ./...` vert ; `packages/` n'existe plus |
